"""Tests for S3 access: credentials and prefix listing.

The screen half of this module lives in
``photo_terminal.terminal.screens.s3_browse`` and is covered by
``tests/terminal/test_s3_browse_screen.py``, which drives it against a fake
port instead of a mocked boto3 session. Composing the two is the pipeline's
job and is covered by ``tests/app/test_wiring.py``.
"""
...
from photo_terminal.storage.s3 import (
    S3FolderLister,
    list_s3_folders,
    validate_s3_access,
)
